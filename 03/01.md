...
- Node.js 코어 함수에서 표준 규칙은 콜백이 맨 마지막 인자로 전달되어야 한다는 것이다.
...
- CPS 함수에서 생성된 오류는 항상 콜백의 첫 번째 인자로 전달된다.
...
- 비동기식 CPS에서 적절한 에러 전파는 호출 체인 다음에 콜백으로 전달하여 수행한다.
...
- 떄때로 비동기 함수의 콜백으로 에러가 밖으로 전달되지 않는 경우가 있다.
- 비동기식 콜백 내부에서 예외를 발생시키면 예외가 이벤트 루프로 이동하여 다음 콜백으로 전파되지 않는다.
...
- 위 함수를 실행하면 에러 메시지가 출력된다.
    - 예외가 콜백에서 스택으로 이동한 다음, 바로 이벤트 루프로 이동하여 throw 된다는 것을 볼 수 있다.
- 이런 경우 예외가 이벤트 루프에 도달하는 순간 애플리케이션은 종료된다.
- Node.js는 프로세스를 종료하기 직전에 uncaughtException이란 특수 이벤트를 내보낸다.
